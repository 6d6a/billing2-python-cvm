...
def main():
    if len(sys.argv) != 2:
        sys.stderr.write("syntax: vmip.py vm_name, ip_address or domain_name\n")
        sys.exit(1)
    ...
    if is_ip(vm):
        ...
        if vm[:2] == "vm":
            ...
            vm_tmp = vm
        result = re.match("(\d{2,7})(\.majordomo\.ru|\.mj-host\.ru)?", vm_tmp)
    ...
    if vm_ip is not None:
        ...
        retcode = xml.vds.get_account(vm_name)
        print(json.dumps(retcode, ensure_ascii=False));

    print(json.dumps(xml.vds.get_passwords(vm_name)))
